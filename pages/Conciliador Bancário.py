        ...
        with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
            for page_idx, page in enumerate(pdf.pages):
                ...
                for top in sorted(linhas_dict.keys()):
                    ...
                    if match_valor:
                        valor_bruto = match_valor.group(1); tipo = match_valor.group(2)
                        valor_float = float(valor_bruto.replace('.', '').replace(',', '.'))
                        coord_box = None
                        for w in linha_words:
                            if valor_bruto in w['text']:
                                coord_box = (page_idx, w['x0'], w['top'], w['x1'], w['bottom']); break
                            ...
                            if tokens:
                                for t in reversed(tokens):
                                    ...
                                    if limpo.isdigit() and len(limpo) >= 4:
                                        entry["Documento"] = limpar_documento_pdf(t); break
                            rows_debitos.append(entry)
                        elif tipo == 'C':
                            hist_upper = texto_sem_valor.upper()
                            if any(x in hist_upper for x in ["TED DEVOLVIDA", "DEVOLUCAO DE TED", "TED DEVOL"]): rows_devolucoes.append(entry)
        df_debitos = pd.DataFrame(rows_debitos); coords_referencia = rows_debitos + rows_devolucoes
...
def processar_excel_detalhado(file_bytes, df_pdf_ref, is_csv=False):
    try:
        ...
        mask_aa_ded = df['Info_AA'].astype(str).str.contains(r"Ded\.|Retenção|PASEP|RFB|Federal", case=False, regex=True, na=False)
        mask_250_restrict = df['Info_Z'].astype(str).str.contains("250", na=False) & df['Info_AB'].astype(str).str.contains("transferência financeira concedida|repasse financeiro concedido", case=False, na=False)
        ...
        df_final['Documento'] = df_final.apply(find_doc, axis=1); df_final['Descricao_Excel'] = df_final['Info_AA']
...
def executar_conciliacao_inteligente(df_pdf, df_excel):
    ...
    for chave in todas_chaves:
        ...
        if s_p > 0 or s_e > 0:
            doc = df_pdf.loc[idxs_p[0], 'Documento'] if len(idxs_p)>0 else df_excel.loc[idxs_e[0], 'Documento']
    ...
    for idx_e, row_e in df_excel.iterrows():
        if idx_e not in idx_e_u: res.append({'Data': row_e['Data'], 'Histórico': "Pendente no Razão", 'Documento': row_e['Documento'], 'Valor_Extrato': 0.0, 'Valor_Razao': row_e['Valor_Razao'], 'Diferença': -row_e['Valor_Razao']})

    df_f = pd.DataFrame(res); df_f['dt'] = pd.to_datetime(df_f['Data'], format='%d/%m/%Y', errors='coerce')
...
def gerar_pdf_final(df_f, titulo_completo):
    ...
    story = []; styles = getSampleStyleSheet(); story.append(Paragraph("Relatório de Conciliação Bancária", styles["Title"]))
    story.append(Paragraph(f"<b>Conta:</b> {titulo_completo.replace('Conciliação ', '')}", ParagraphStyle(name='C', alignment=1))); story.append(Spacer(1, 15))
    data = [['Data', 'Documento', 'Vlr. Extrato', 'Vlr. Razão', 'Diferença']]
    table_style = [('GRID', (0,0), (-1,-1), 0.5, colors.black), ('BACKGROUND', (0,0), (-1,0), colors.black), ('TEXTCOLOR', (0,0), (-1,0), colors.white), ('ALIGN', (2,0), (-1,-1), 'RIGHT')]
...
def gerar_extrato_marcado(pdf_bytes, df_f, coords_referencia, nome_original):
    doc = fitz.open(stream=pdf_bytes, filetype="pdf"); divergencias = df_f[abs(df_f['Diferença']) >= 0.01]
    for _, erro in divergencias.iterrows():
        for item in coords_referencia:
            if item['Data'] == erro['Data'] and abs(item['Valor_Extrato'] - erro['Valor_Extrato']) < 0.01:
                if item['coords']:
                    pno, x0, top, x1, bottom = item['coords']; page = doc[pno]; rect = fitz.Rect(x0-2, top-2, x1+2, bottom+2)
...
if st.button("PROCESSAR CONCILIAÇÃO", use_container_width=True):
    if up_pdf and up_xlsx:
        with st.spinner("Processando..."):
            ...
            df_p, coords_ref = processar_pdf(pdf_bytes); df_e = processar_excel_detalhado(xlsx_bytes, df_p, is_csv=up_xlsx.name.endswith('csv'))
