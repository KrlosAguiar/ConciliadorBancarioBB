...
def executar_conciliacao_inteligente(df_pdf, df_excel):
    """
    LOGICA AJUSTADA: Consolidação absoluta para FUNDEB e PASEP.
    """
    res = []
    idx_p_u = set()
    idx_e_u = set()
    ...
    # ==========================================================================
    # 0. AGRUPAMENTO TOTAL: FUNDEB E PASEP
    # ==========================================================================
    for cat in ["FUNDEB", "PASEP"]:
        ...
        p_groups = df_pdf[mask_p].groupby(['Data', 'Doc_Clean'])
        e_groups = df_excel[mask_e].groupby(['Data', 'Doc_Clean'])
        
        todas_chaves = set(p_groups.groups.keys()) | set(e_groups.groups.keys())
        
        for chave in todas_chaves:
            indices_p = p_groups.groups.get(chave, [])
            indices_e = e_groups.groups.get(chave, [])
            ...
            if soma_p > 0 or soma_e > 0:
                doc_exibir = df_pdf.loc[indices_p[0], 'Documento'] if len(indices_p) > 0 else df_excel.loc[indices_e[0], 'Documento']
                res.append({
                    'Data': chave[0],
                    'Histórico': f"Ded. {cat} (Total Agrupado)",
                    'Documento': doc_exibir,
                    'Valor_Extrato': soma_p,
                    'Valor_Razao': soma_e,
                    'Diferença': round(soma_p - soma_e, 2)
                })
                idx_p_u.update(indices_p); idx_e_u.update(indices_e)
    ...
    for idx_p, row_p in df_pdf.iterrows():
        ...
        if not cand.empty:
            idx_e = cand.index[0]
            res.append({'Data': row_p['Data'], 'Histórico': row_p['Histórico'], 'Documento': "Docs dif.", 'Valor_Extrato': row_p['Valor_Extrato'], 'Valor_Razao': cand.loc[idx_e]['Valor_Razao'], 'Diferença': 0.0})
            idx_p_u.add(idx_p); idx_e_u.add(idx_e)
