...
def gerar_excel_geral(df_resumo, ug):
    out = io.BytesIO()
    with pd.ExcelWriter(out, engine='xlsxwriter') as writer:
        df_resumo.to_excel(writer, sheet_name='Geral', index=False)
        wb = writer.book
        ...
        ws.set_column(0, 0, 40, fmt_text) # Conta
        ws.set_column(1, 4, 18, fmt_money) # Valores
        ...
        for row_num, row_data in enumerate(df_resumo.values):
            for col_num, cell_data in enumerate(row_data):
                if col_num == 0:
                    ws.write(row_num + 1, col_num, cell_data, fmt_text)
                else:
                    style = fmt_money
                    if col_num == 4: # Saldo a Pagar
                        style = fmt_green if cell_data >= 0 else fmt_red
                    ws.write(row_num + 1, col_num, cell_data, style)
    return out.getvalue()
...
def gerar_pdf_geral(df_resumo, ug):
    ...
    story.append(Paragraph(f"Relatório Geral de Retenções - {ug}", styles["Title"]))
    story.append(Spacer(1, 5*mm))
    
    data = [df_resumo.columns.tolist()]
    for _, row in df_resumo.iterrows():
        r_data = [
            row['Conta De Retenção'],
    ...
    table_style = [
        ...
        ('ALIGN', (0,1), (0,-1), 'LEFT'), # Conta align left
        ('ALIGN', (1,1), (-1,-1), 'RIGHT'), # Values align right
        ('FONTSIZE', (0,0), (-1,-1), 8),
    ...
    for i in range(1, len(data)):
        ...
        color = colors.darkgreen if val_saldo >= 0 else colors.red
        table_style.append(('TEXTCOLOR', (4, i), (4, i), color))
        table_style.append(('FONTNAME', (4, i), (4, i), 'Helvetica-Bold'))

    t = Table(data, colWidths=[70*mm, 30*mm, 30*mm, 30*mm, 30*mm])
    t.setStyle(TableStyle(table_style))
    story.append(t)
    doc.build(story)
    return buffer.getvalue()
...
if arquivo:
    ...
    if not df_dados.empty:
        ...
        # --- ESTADO DE NAVEGAÇÃO ---
        if 'modo_geral' not in st.session_state: st.session_state['modo_geral'] = False
        if 'df_saldos_geral' not in st.session_state:
            # Inicializa tabela com contas e saldo 0
        ...
        if st.button("PROCESSAR CONCILIAÇÃO INDIVIDUAL", use_container_width=True):
            st.session_state['modo_geral'] = False
        
        # Lógica Individual
        if not st.session_state['modo_geral']:
            conta_sel = st.selectbox("Conta de Retenção", opcoes_conta)
            val_anterior_str = st.text_input("Saldo Anterior", value="0,00", help="Digite o saldo acumulado.")
            
            if st.button("EXECUTAR CONCILIAÇÃO INDIVIDUAL", use_container_width=True):
                 ...
                 if not df_res.empty:
                    # [VISUALIZAÇÃO INDIVIDUAL - CÓDIGO EXISTENTE MANTIDO]
        ...
        if st.button("PROCESSAR CONCILIAÇÃO GERAL", use_container_width=True):
            st.session_state['modo_geral'] = True
        
        # Lógica Geral
        if st.session_state['modo_geral']:
            ...
            # Tabela Editável para Saldos Anteriores
            ...
            if st.button("CONCILIAR", type="primary", use_container_width=True):
                ...
                for idx, row in st.session_state['df_saldos_geral'].iterrows():
                    ...
                    # Reaproveita a lógica de processamento
                ...
                st.dataframe(
                    ...
                    use_container_width=True, 
                    column_config={
                        "Saldo Anterior": st.column_config.NumberColumn(format="R$ %.2f"),
                        "Retido Período": st.column_config.NumberColumn(format="R$ %.2f"),
                        "Pago Período": st.column_config.NumberColumn(format="R$ %.2f"),
                        "Saldo A Pagar": st.column_config.NumberColumn(format="R$ %.2f")
                    },
                    hide_index=True
                )
                
                # Botões de Download GERAL
                ug_limpa = limpar_nome_arquivo(str(ug_sel))
